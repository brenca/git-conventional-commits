...
async function getLastVersionCommit() {
  ...
  const commits = await execAsync(`git log --oneline`, {encoding: 'UTF-8'})
  const versionRegExp = /^([a-fA-F0-9]{8}) version: v?([0-9]+\.[0-9]+\.[0-9]+)/
  const lastVersion = commits.split('\n').find(commit => versionRegExp.test(commit))

  const data = lastVersion.match(versionRegExp)
...
module.exports = {
  ...
  getLastVersionCommit: getLastVersionCommit
};
