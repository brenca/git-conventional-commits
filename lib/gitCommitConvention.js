...
module.exports = function(convention, commitAnchor = 'HEAD') {
  ...
  async function getLastVersionCommit() {
    if(lastVersionCommit === undefined){
      lastVersionCommit = (await Git.getLastVersionCommit())['version']
    }
  ...
  async function getCommitLogCommitVersion() {
    if(commitLog === undefined){
      const lastReleaseCommit = (await Git.getLastVersionCommit())['hash']
      const commits = await Git.commitLog(lastReleaseCommit, commitAnchor);
      commitLog = commits.map(parseCommit);
    }
    return commitLog
  }
  ...
  async function getVersion() {
    ...
    const lastReleaseTag = await getLastVersionCommit();
    if (lastReleaseTag) {
    ...
    (await getCommitLogCommitVersion()).forEach(commit => {
  ...
  this.commitLog = getCommitLogCommitVersion;
